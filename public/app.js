...
async function getStoredReview(id = '') {
  return libraryRequest('readonly', (store) => store.get(id));
}

async function putStoredReview(review = {}) {
...
async function runOcr(file) {
  ...
  if (!response.ok) throw new Error(data?.error || `OCR request failed (${response.status}).`);
...
async function resolveReferencesWithCompletion(referenceBlocks = [], options = {}) {
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok) throw new Error(data?.error || `Reference resolver failed (${response.status}).`);
...
async function resolveCountsWithCompletion(blocks = []) {
  const started = runtimeNow();
  const response = await fetch('/api/resolve-counts', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ blocks })
  });
  const data = await response.json().catch(() => null);
  if (!response.ok) throw new Error(data?.error || `Counted-text resolver failed (${response.status}).`);
...
async function resolveDisplayItemsWithCompletion(displayItems = [], bodyBlocks = []) {
  const started = runtimeNow();
  const response = await fetch('/api/resolve-display-items', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ displayItems, bodyBlocks })
  });
  const data = await response.json().catch(() => null);
  if (!response.ok) throw new Error(data?.error || `Table/figure resolver failed (${response.status}).`);
