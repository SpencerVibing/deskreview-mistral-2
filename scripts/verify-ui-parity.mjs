...
async function assertCustomizeChecksModal(page) {
  ...
  await assertVisible(page, '#readerCustomizeTabContent');
  await assertText(page, '#readerCustomizeFacets', /Article counts/i);
  await assertText(page, '#readerCustomizeFacets', /Essential guides/i);
  await assertText(page, '#readerCustomizeFacets', /Reporting guidelines/i);
  await assertText(page, '#readerCustomizeFacets', /Open Science/i);
  await assertText(page, '#readerCustomizeFacets', /Custom guidelines/i);
  await assertText(page, '#readerCustomizeFacets', /Reader functions/i);
  assert.equal(await page.locator('#customizeChecksModal [data-bs-toggle="pill"]').count(), 6, 'Customize checks modal should use a facet side nav.');
  assert.equal(await page.locator('#readerCustomizeEssentialPane.show.active').count(), 1, 'Customize checks should default to the Essential guides pane.');
  await assertText(page, '#readerCustomizeEssentialPane', /Essential guidelines/i);
  ...
  await page.click('#readerCustomizeCountsTab');
  await page.waitForSelector('#readerCustomizeCountsPane.show.active', { timeout: 10000 });
  await assertText(page, '#readerCustomizeCountsPane', /Article element counts/i);
  await page.click('#readerCustomizeReportingTab');
  await page.waitForSelector('#readerCustomizeReportingPane.show.active', { timeout: 10000 });
  await assertText(page, '#readerCustomizeReportingPane', /Reporting guidelines/i);
  await assertVisible(page, '#readerCustomizeReportingPane [data-plugin-toggle="reporting-guidelines"]');
  await page.click('#readerCustomizeCustomTab');
  await page.waitForSelector('#readerCustomizeCustomPane.show.active', { timeout: 10000 });
  await assertText(page, '#readerCustomizeCustomPane', /Custom guidelines/i);
  await assertVisible(page, '#readerCustomizeCustomPane [data-plugin-toggle="custom-guidelines"]');
  await assertVisible(page, '#readerCustomizeCustomPane [data-custom-guide-form]');
  await page.click('#readerCustomizeFunctionsTab');
  await page.waitForSelector('#readerCustomizeFunctionsPane.show.active', { timeout: 10000 });
  await assertText(page, '#readerCustomizeFunctionsPane', /Reader functions/i);
  await page.keyboard.press('Escape');
